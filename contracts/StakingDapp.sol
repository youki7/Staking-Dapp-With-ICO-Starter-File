...
contract StakingDapp is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        uint256 amount;
        uint256 lastRewardAt; // lastRewardAt field also means the datetime he deposit the token
        uint256 lockUntil;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        //if pid not exist, it will throw an index exceeded error
        //if user not exist, mapping will return a default struct
    ...
    function withdraw(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        require(user.amount > _amount, "Withdraw amount exceeds the blance");
    ...
    function _calcPendingReward(
        ...
        PoolInfo storage pool = poolInfo[_pid];
        // uint256 daysPasse = (block.timestamp - _user.lockDays) / 864000;
        uint256 daysPassed = (block.timestamp - _user.lastRewardAt) / 60;

        if (daysPassed > pool.lockDays) {
            daysPassed = pool.lockDays;
        }

        return ((_user.amount * daysPassed) / 365 / 100) * pool.apy;
    }
    ...
    function sweep(address token, uint256 amount) external onlyOwner {
        ...
        require(amount <= tokenBalance, "Amount exceed balance");
        require(
            tokenBalance - amount >= depositedTokens[token],
    ...
    function modifyPool(uint256 _pid, uint256 _apy) external onlyOwner {
        PoolInfo storage pool = poolInfo[_pid];
        pool.apy = _apy;
    }
