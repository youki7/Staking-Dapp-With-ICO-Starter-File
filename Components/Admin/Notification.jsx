...
const Notification = ({ index, notify, poolDetails }) => {
  ...
  useEffect(() => {
    const calculatePercentage = () => {
      ...
      if (percentageNew === 0) {
        console.log('token balance is 0 , cannot calculate percentage')
        return
      }
      setPercentage(percentageNew)
    }

    const interval = setTimeout(calculatePercentage, 1000)
  ...
  return (
    <div key={index} className="deposit">
      <div className="deposit__name">
        ...
        <h3 className="deposit__title">{notify?.message}</h3>
      </div>
      ...
      >
        ...
        <div className="progressbar__values">
          <span className="progressbar__value progressbar_value--left" style={{visibility:'hidden'}}>0%</span>
          <span className="progressbar__value progressbar_value--right">100%</span>
        </div>
      </div>
